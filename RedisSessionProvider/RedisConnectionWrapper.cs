using RedisSessionProvider.redisWrapper;
using System;
        ...
        public RedisConnectionWrapper(ProviderConfiguration configuration, string id)
        {
            ...
            if (sharedConnection == null)
            {
                lock (lockForSharedConnection)
                {
                    if (sharedConnection == null)
                    {
                        //sharedConnection = new RedisSharedConnection(configuration, () => new SRSRedisClientConnection(configuration));
                        sharedConnection = new RedisSharedConnection(configuration, () => new RedisClientConnection(configuration));
